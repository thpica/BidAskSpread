...
/// move constructor
MemoryMapped::MemoryMapped(MemoryMapped&& other):
	_filename(std::move(other._filename)),
	_filesize(std::move(other._filesize)),
	_hint(std::move(other._hint)),
	_mappedBytes(std::move(other._mappedBytes)),
	_file(std::move(other._file)),
#ifdef _MSC_VER
	_mappedFile(std::move(other._mappedFile)),
#endif
	_mappedView(std::move(other._mappedView))
{}
...
MemoryMapped& MemoryMapped::operator=(MemoryMapped&& other){
	_filename = std::move(other._filename);
	_filesize = std::move(other._filesize);
	_hint = std::move(other._hint);
	_mappedBytes = std::move(other._mappedBytes);
...
const unsigned char MemoryMapped::operator[](size_t offset) const
{
...
const unsigned char MemoryMapped::at(size_t offset) const
{
  // checks
  if (!_mappedView)
